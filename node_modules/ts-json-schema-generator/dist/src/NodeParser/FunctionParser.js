...
class FunctionParser {
    ...
    supportsNode(node) {
        ...
        return node.kind === typescript_1.default.SyntaxKind.ArrowFunction && typescript_1.default.isVariableDeclaration(node.parent);
    }
    createType(node, context) {
        const parameterTypes = node.parameters.map((parameter) => {
            return this.childNodeParser.createType(parameter, context);
        });
        const namedArguments = new ObjectType_1.ObjectType(`object-${nodeKey_1.getKey(node, context)}`, [], parameterTypes.map((parameterType, index) => {
